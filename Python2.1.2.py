...
class report():
    def __init__(self,professionName, vacbyyear, countbyyear, vacbyyearprof, vacbyyearprofcount, vacbytown, citydictwithpart):
            self.professionName = professionName
...
class creategraph():
    ...
    def makegraph(self):
        ...
        #lists = sorted(self.vacbyyear.items()) # sorted by key, return a list of tuples
        ...
        labels2 = list(self.vacbytown.keys())
        #Иначе неверный порядок
        labels2.reverse()
        labels2 = [sub.replace(' ', '\n') for sub in labels2]
        labels2 = [sub.replace('-', '-\n') for sub in labels2]

        salary_level =list(self.vacbytown.values())
...
#fileName = "vacancies_by_year_super_breef.csv"
#fileName = "vacancies_by_year_breef.csv"
...
creategraph(professionName, vacbyyear, countbyyear, vacbyyearprof, vacbyyearprofcount, vacbytown, citydictwithpart)
